...
echo "=============================================="
echo ""
echo "  部署到 Win11 Agent:"
echo ""
echo "  1. 传输文件到 Win11:"
echo "     scp deploy/win-agent/honeypot-agent.exe user@<WINDOWS_IP>:C:/Laji-HoneyPot/"
echo "     scp deploy/win-agent/config.yaml user@<WINDOWS_IP>:C:/Laji-HoneyPot/"
echo "     scp deploy/win-agent/deploy.ps1 user@<WINDOWS_IP>:C:/Laji-HoneyPot/"
echo ""
echo "     (或通过 U盘 / 共享文件夹 / 微信 等方式传输)"
echo ""
echo "  2. 重要: 修改 config.yaml 中的 manager_addr"
echo "     将 MANAGER_IP_PLACEHOLDER 替换为 macOS M1 本机的局域网IP"
