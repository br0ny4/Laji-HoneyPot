...
resp=$(curl -s -X POST "$MGR/api/countermeasure/exfil" \
  ...
  -d '{"type":"screen_capture","target_ip":"10.111.29.4","data_type":"screen_capture","data":{"width":1920,"height":1080,"dpr":2,"captured_at":"2026-07-02T10:00:00Z"}}')
echo "  Response: $resp"
...
resp=$(curl -s -X POST "$MGR/api/countermeasure/exfil" \
  -H "Content-Type: application/json" \
  -d '{"type":"file_scan","target_ip":"10.111.29.4","data_type":"file_scan","data":{"tool_dirs":{"burpsuite":"C:\\Tools\\BurpSuite","nmap":"C:\\Tools\\Nmap","sqlmap":"/opt/sqlmap","hydra":"/usr/bin/hydra","metasploit":"C:\\metasploit-framework"},"sensitive_files":["passwords.txt","config.yaml","ssh_key","token.txt"],"clipboard":"admin:Password123!"}}')
echo "  Response: $resp"
...
resp=$(curl -s -X POST "$MGR/api/countermeasure/exfil" \
  -H "Content-Type: application/json" \
  -d '{"type":"net_probe","target_ip":"10.111.29.4","data_type":"net_probe","data":{"internal_ips":["10.111.29.5","10.111.29.6","192.168.1.100"],"peer_assets":[{"ip":"10.111.29.5","open_ports":[22,3389,8080],"services":["ssh","rdp"],"role":"attacker_workstation","confidence":0.85},{"ip":"10.111.29.6","open_ports":[80,443,22],"services":["http","https","ssh"],"role":"command_node","confidence":0.72}]}}')
echo "  Response: $resp"
...
resp=$(curl -s -X POST "$MGR/api/countermeasure/exfil" \
  -H "Content-Type: application/json" \
  -d '{"type":"screen_capture","target_ip":"10.111.29.4","data_type":"screen_capture","data":{"width":1920,"height":1080}}')
